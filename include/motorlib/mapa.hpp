...
private:
	...
	void colorTopologyMM(unsigned char altura);
	void colorCeldaOpuestoMM(unsigned char celda);
	void colorCeldaOpuestoMM2(unsigned char celda);

	void drawIconoCeldaMM(unsigned char celda, unsigned char altura, int i, int j);
	void renderHeightValue(unsigned char altura);

	void complementosCelda(unsigned char celda, unsigned char altura, int i, int j, int level);
	bool esTuberia(unsigned char c);
...
public:
	...
	void drawMM1(vector<unsigned int> objetivosActivos, int level, bool drawTopology = false);
	void drawMM2(vector<unsigned int> objetivosActivos, int level, bool drawTopology = false, int mapMode = 0);
	void drawFirstPerson(int entidad, vector<unsigned int> objetivosActivos, int level);
	void drawZenithal(int entidad, vector<unsigned int> objetivosActivos, int level, float zoom = 55.0f);
	...
	void setAltura(unsigned int i, unsigned int j, unsigned char valor) { mapaAlturas[i][j] = valor; }
    ...
    void setDrawGridFP(bool val) { drawGridFP = val; }
};
#endif
