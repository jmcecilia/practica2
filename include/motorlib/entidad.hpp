...
enum SubTipo { ingeniero, tecnico, excursionista, vandalo, __NONE__ };
...
public:
  Entidad(Tipo paramTipo, SubTipo paramSubTipo, Orientacion paramOrient,
          unsigned int paramF, unsigned int paramC, Objeto3D *mod,
          Comportamiento *comp, int numberdest,
          vector<unsigned int> CasillasObjetivo, int bateria, int pasos) {
    tipo = paramTipo;
  ...
  // void setObjetivo(unsigned int paramF, unsigned int paramC) {destF = paramF;
  // destC = paramC;}
  ...
  Comportamiento *getComportamiento() { return comportamiento; }

  vector<vector<unsigned char>> getMapaResultado() {
    return comportamiento->getMapaResultado();
  }
  vector<vector<unsigned char>> getMapaEntidades() {
    return comportamiento->getMapaEntidades();
  }
  ListaCasillasPlan getMapaPlan() { return comportamiento->getMapaPlan(); }
  ListaCasillasPlan getCanalizacionPlan() {
    return comportamiento->getCanalizacionPlan();
  }
  ...
  int fixBateria_sig_accion_jugador(unsigned char celdaJugador, int difAltura,
                                    Action accion);
  int fixBateria_sig_accion_colaborador(unsigned char celdaColaborador,
                                        Action accion);
  ...
  void AsignarCall_ON(bool valor) {
    venpaca = valor;
    if (!valor) {
      gotoF = -1;
      gotoC = -1;
  ...
  void setGoto(int f, int c) {
    gotoF = f;
    gotoC = c;
  }
  int getGotoF() const { return gotoF; }
  int getGotoC() const { return gotoC; }
  void setEnfrente(bool valor) { enfrente = valor; }
  bool getEnfrente() const { return enfrente; }
